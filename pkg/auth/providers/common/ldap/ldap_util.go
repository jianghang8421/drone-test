...
func GatherParentGroups(groupPrincipal v3.Principal, searchDomain string, groupScope string, config *ConfigAttributes, lConn *ldapv2.Conn,
	...
	for i := 0; i < len(resultGroups.Entries); i++ {
		...
		principal, err := AttributesToPrincipal(entry, entry.DN, groupScope, config.ProviderName, config.UserObjectClass, config.UserNameAttribute, config.UserLoginAttribute, config.GroupObjectClass, config.GroupNameAttribute, "", config.GroupUniqueIDAttribute)
		if err != nil {
