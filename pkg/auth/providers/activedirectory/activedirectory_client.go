...
func (p *adProvider) getPrincipalsFromSearchResult(result *ldapv2.SearchResult, config *v3.ActiveDirectoryConfig, lConn *ldapv2.Conn) (v3.Principal, []v3.Principal, error) {
	...
	user, err := ldap.AttributesToPrincipal(entry, result.Entries[0].DN, scopeType, Name, config.UserObjectClass, config.UserNameAttribute, config.UserLoginAttribute, config.GroupObjectClass, config.GroupNameAttribute, config.UserUniqueIDAttribute, config.GroupUniqueIDAttribute)
	if err != nil {
		return userPrincipal, groupPrincipals, err
	}
	...
	if config.NestedGroupMembershipEnabled != nil {
		if *config.NestedGroupMembershipEnabled == true {
			...
			commonConfig := ldap.ConfigAttributes{
				...
				GroupUniqueIDAttribute:      config.GroupUniqueIDAttribute,
			}
			...
			searchAttributes := []string{MemberOfAttribute, ObjectClass, config.GroupObjectClass, config.UserLoginAttribute, config.GroupNameAttribute,
				config.GroupSearchAttribute, config.GroupUniqueIDAttribute}
...
func (p *adProvider) getGroupPrincipalsFromSearch(searchBase string, filter string, config *v3.ActiveDirectoryConfig, lConn *ldapv2.Conn,
	...
	for _, e := range result.Entries {
		principal, err := ldap.AttributesToPrincipal(e, e.DN, scopeType, Name, config.UserObjectClass, config.UserNameAttribute, config.UserLoginAttribute, config.GroupObjectClass, config.GroupNameAttribute, config.UserUniqueIDAttribute, config.GroupUniqueIDAttribute)
		if err != nil {
...
func (p *adProvider) getPrincipal(distinguishedName string, scope string, config *v3.ActiveDirectoryConfig, caPool *x509.CertPool) (*v3.Principal, error) {
	...
	if err != nil {
		if ldapv2.IsErrorWithCode(err, ldapv2.LDAPResultInvalidCredentials) && config.Enabled {
			var kind string
			if strings.EqualFold(UserScope, scope) || strings.EqualFold(UserUIDScope, scope) {
				kind = "user"
			} else if strings.EqualFold(GroupScope, scope) || strings.EqualFold(GroupUIDScope, scope) {
				kind = "group"
			}
	...
	principal, err := ldap.AttributesToPrincipal(entry, distinguishedName, scope, Name, config.UserObjectClass, config.UserNameAttribute, config.UserLoginAttribute, config.GroupObjectClass, config.GroupNameAttribute, config.UserUniqueIDAttribute, config.GroupUniqueIDAttribute)
	if err != nil {
		return nil, err
	}
	return principal, nil
...
func (p *adProvider) searchLdap(query string, scope string, config *v3.ActiveDirectoryConfig, lConn *ldapv2.Conn) ([]v3.Principal, error) {
	...
	for i := 0; i < len(results.Entries); i++ {
		...
		principal, err := ldap.AttributesToPrincipal(entry, entry.DN, scopeType, Name, config.UserObjectClass, config.UserNameAttribute, config.UserLoginAttribute, config.GroupObjectClass, config.GroupNameAttribute, config.UserUniqueIDAttribute, config.GroupUniqueIDAttribute)
		if err != nil {
