	...
	for _, r := range recipients {
		if r.NotifierName != "" {
			...
			if notifier.Spec.PagerdutyConfig != nil {
				...
				err := d.syncReceiver(DingTalk, r.NotifierName, notifier.Spec)
				if err != nil {
					logrus.Errorf("Failed to sync receiver : %v", err)
					continue
				}
				...
				err := d.syncReceiver(MicrosoftTeams, r.NotifierName, notifier.Spec)
				if err != nil {
					logrus.Errorf("Failed to sync receiver : %v", err)
					continue
				}
				...
				err := d.syncReceiver(AliyunSMS, r.NotifierName, notifier.Spec)
				if err != nil {
					logrus.Errorf("Failed to sync receiver : %v", err)
					continue
				}
...
func (d *ConfigSyncer) syncReceiver(notifierType, notifierName string, notifierSpec v3.NotifierSpec) error {
	...
	if notifierType == DingTalk {
		config.Providers[notifierName] = &Provider{
			Type:       DingTalk,
			WebHookURL: notifierSpec.DingtalkConfig.URL,
		}

		if notifierSpec.DingtalkConfig.Secret != "" {
			config.Providers[notifierName].Secret = notifierSpec.DingtalkConfig.Secret
		}

		if notifierSpec.DingtalkConfig.HTTPClientConfig != nil {
			config.Providers[notifierName].ProxyURL = notifierSpec.DingtalkConfig.HTTPClientConfig.ProxyURL
		}

		config.Receivers[notifierName] = &Receiver{
			Provider: notifierName,
		}
	} else if notifierType == MicrosoftTeams {
		config.Providers[notifierName] = &Provider{
			Type:       MicrosoftTeams,
			WebHookURL: notifierSpec.MSTeamsConfig.URL,
		}

		if notifierSpec.MSTeamsConfig.HTTPClientConfig != nil {
			config.Providers[notifierName].ProxyURL = notifierSpec.MSTeamsConfig.HTTPClientConfig.ProxyURL
		}

		config.Receivers[notifierName] = &Receiver{
			Provider: notifierName,
		}
	} else if notifierType == AliyunSMS {
		config.Providers[notifierName] = &Provider{
			Type:            AliyunSMS,
			AccessKeyID:     notifierSpec.AliyunSMSConfig.AccessKeyID,
			AccessKeySecret: notifierSpec.AliyunSMSConfig.AccessKeySecret,
			SignName:        notifierSpec.AliyunSMSConfig.SignName,
			TemplateCode:    notifierSpec.AliyunSMSConfig.TemplateCode,
		}

		config.Receivers[notifierName] = &Receiver{
			Provider: notifierName,
			To:       notifierSpec.AliyunSMSConfig.To,
		}
	}
