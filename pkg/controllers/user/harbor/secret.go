...
func (c *Controller) syncSettings(key string, obj *v3.Setting) (runtime.Object, error) {
	// only sync harbor registry secret when HarborServerURL and HarborAdminAuth changed
	if settings.HarborAdminAuth.Name == key || settings.HarborServerURL.Name == key {
		harborServerStr := settings.HarborServerURL.Get()
		auth := settings.HarborAdminAuth.Get()

		if harborServerStr != "" && auth != "" {
			if settings.HarborAdminAuth.Name == key {
				return c.syncSecret(obj.Value, "", false)
			} else if settings.HarborServerURL.Name == key {
...
func (c *Controller) clearUserAnnotations(key string, obj *v3.Setting) (runtime.Object, error) {
	if settings.HarborAdminAuth.Name == key || settings.HarborServerURL.Name == key {
		harborServerStr := settings.HarborServerURL.Get()
		auth := settings.HarborAdminAuth.Get()
		if auth == "" || harborServerStr == "" {
			users, _ := c.users.List(metav1.ListOptions{})
			for _, user := range users.Items {
				if user.Annotations != nil && user.Annotations[harborUserAnnotationAuth] != "" && user.Annotations[harborUserAnnotationEmail] != "" {
